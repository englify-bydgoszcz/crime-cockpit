(() => {
  ...
  function characterTrailHtml_(characterId,bookId,name){
    const trace=rowForCharacter('characterEncounterTrace',characterId,bookId)||null;
    if(!trace){
    ...
    const checkpoint=Math.max(1,Number(String(cell(trace,'Checkpoint')||'').replace(/[^0-9.]/g,''))||0);
    const first=Math.max(1,Number(cell(trace,'First Page'))||1),last=Math.max(first,Number(cell(trace,'Last Page'))||first);
    const pages=String(cell(trace,'Mention Pages')||'').split(',').map(x=>Number(String(x).trim())).filter(n=>Number.isFinite(n)&&n>=1&&n<=checkpoint);
    const unique=[...new Set(pages)].sort((a,b)=>a-b);
    const dots=unique.map((page,idx)=>{
      const left=checkpoint<=1?0:((page-1)/(checkpoint-1))*100;
    ...
    const firstPos=checkpoint<=1?0:((first-1)/(checkpoint-1))*100;
    ...
    return `<section class="casefile-paper-section character-trail-section"><div class="section-kicker">ŚLAD POSTACI · DO CHECKPOINTU</div><div class="character-trail-stats"><div><span>Pierwszy ślad</span><strong>p${first}</strong></div><div><span>Ostatni ślad</span><strong>p${last}</strong></div><div><span>Od ostatniego</span><strong>${Number.isFinite(since)?since:'—'} str.</strong></div><div><span>Jawne wzmianki</span><strong>${count}</strong></div></div><div class="character-trail-axis" role="img" aria-label="Ślad postaci ${esc(name)} od strony 1 do ${checkpoint}"><div class="character-trail-line"></div><div class="character-trail-window" style="left:${firstPos.toFixed(3)}%;width:${Math.max(.7,lastPos-firstPos).toFixed(3)}%"></div>${dots}<span class="character-trail-start">1</span><span class="character-trail-end">${checkpoint}</span></div><div class="character-trail-legend"><span><i class="first"></i>pierwszy</span><span><i></i>wzmianka</span><span><i class="last"></i>ostatni</span></div><p class="small-note">Ślad opiera się wyłącznie na jawnych wzmiankach nazwiska/imienia w potwierdzonym prefiksie. Gęstość kropek nie oznacza ważności, podejrzenia ani winy.</p></section>`;
  }
